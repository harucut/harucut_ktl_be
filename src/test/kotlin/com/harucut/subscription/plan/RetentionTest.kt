...
class RetentionTest {
    ...
    @Test
    @DisplayName("Months는 now 기준 N달 전 cutoff를 만들고 그 이후 생성물만 접근 가능하다")
    fun months() {
        val retention = Retention.Months(3)

        assertThat(retention.cutoffFrom(now)).isEqualTo(now.minusMonths(3))
        assertThat(retention.isAccessible(now.minusMonths(2), now)).isTrue()
        assertThat(retention.isAccessible(now.minusMonths(4), now)).isFalse()
        assertThat(retention.isAccessible(null, now)).isTrue()
    }
