...
data class Response<T>(
    ...
    companion object {
        ...
        fun <T> errorResponse(errorCode: ErrorCode): Response<T> =
            Response(code = errorCode.code, status = errorCode.httpStatus.value(), message = errorCode.message)

        fun <T> from(errorCode: ErrorCode, data: T): Response<T> =
            Response(code = errorCode.code, status = errorCode.httpStatus.value(), message = errorCode.message, data = data)
    }
}